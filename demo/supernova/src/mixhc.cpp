...
static Mix MIX[] = {
    { "kick",  0.62f,  0.00f, 0.00f },
    { "bass",  0.86f,  0.00f, 0.72f },
    { "lead",  2.30f,  0.00f, 0.20f },   /* the song. barely ducked.       */
    { "arp",   0.82f,  0.24f, 0.45f },
    { "stab",  1.60f, -0.18f, 0.50f },
    { "piano", 1.35f, -0.08f, 0.40f },
    { "pad",   0.60f,  0.00f, 0.45f },
    { "clap",  1.35f,  0.06f, 0.25f },
    { "hatcl", 0.95f,  0.22f, 0.30f },
    { "hatop", 1.05f, -0.26f, 0.35f },
    { "roll",  2.10f,  0.00f, 0.20f },
    { "crash", 1.25f, -0.05f, 0.30f },
    { "riser", 1.70f,  0.00f, 0.25f }
};
...
int main(int argc, char **argv)
{
    ...
    size_t frames = 0;
    for (int i = 0; i < N; i++) {
        if (!readWav(dir + "/hc-" + MIX[i].name + ".wav", lay[(size_t)i])) {
    ...
    std::vector<float> duck(frames, 0.0f);
    {
        ...
        const double rel = 1.0 - std::exp(-1.0 / (0.090 * RATE));
        double e = 0.0, mx = 0.0;
    ...
    writeWav(dir + "/supernova.wav", mix);
    std::printf("\nwritten: %s/starfall.wav\n", dir.c_str());
    return 0;
}
