...
void presetBowed(Builder &b)
{
    ...
    /* Longer than it looks like it needs. The bow does supply the energy,
     * but a short string is a lossy one and at the bottom of the range the
     * bow could not keep up - a held low C faded while it was still being
     * played. */
    b.set(str, 2, 0.78f);        /* DECAY                                     */
    ...
    /* Soft in and out, but not sluggish. The string takes its own moment to
     * speak on top of this, and 0.18 s of envelope on top of that made the
     * note arrive noticeably after the key. */
    b.set(envA, 0, 0.05f);
    b.set(envA, 1, 0.20f);
    b.set(envA, 2, 0.90f);
    b.set(envA, 3, 0.35f);
